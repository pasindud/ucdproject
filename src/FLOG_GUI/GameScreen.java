      ...
      public void run() {
        ...
        try {
          ...
          try {
            ...
            String message = Utils.COMMAND_CODES.CLIENT_JOIN_GAME_CODE + " " + username;
          } catch (Exception e) {
            e.printStackTrace();
          }
        } catch (Exception ex) {
      ...
      case SERVER_PENALIZE_WEKEST:
        if (segments.length == 3) {
          chatFrame.updateMessages(
              "Server", "@" + segments[1] + " reduced 5% from @" + segments[2]);
      ...
      if (userMarks.get(0).getName().equals(username)) {
        ...
        int res =
            JOptionPane.showConfirmDialog(
                null,
                "Do you want to reduce 5% marks from the weakest player?",
                " Punishing Weakest Player",
