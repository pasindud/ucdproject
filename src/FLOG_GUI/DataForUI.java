...
public class DataForUI {
    ...
    public DataForUI() {
        ...
        try {
            URL fontUrl = new URL("");
            LCD = Font.createFont(Font.TRUETYPE_FONT, fontUrl.openStream()).deriveFont(Font.PLAIN, 100);
            
            /*LCD
                    = Font.createFont(Font.TRUETYPE_FONT, new FileInputStream(new File("LCD.tff")))
                    .deriveFont(Font.PLAIN, 100);*/
        } catch (FontFormatException ex) {
            System.out.println(ex.getMessage().toString());
        } catch (IOException ex) {
            LCD = new Font("Arial Rounded MT Bold", 0, 60);
            System.out.println(ex.getMessage().toString()+" font case");
        }
    }
